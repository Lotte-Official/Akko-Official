    ...
    async run(client, message, args) {
        ...
        if (response3.content === 'react') {
            const m = await channel.send(new Discord.MessageEmbed()
                .setColor("ORANGE")
                                         .setImage("https://i.imgur.com/HBxoSP2.gif")
                .setDescription(`This server is protected by [Bonez](${client.unicron.serverInviteURL} 'Bonez's Support Server'), a powerful bot that prevents servers from being raided, React ${await client.getEmoji('yes')} to get yourself verified!`)
            );
